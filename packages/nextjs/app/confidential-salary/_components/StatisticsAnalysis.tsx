...
const RechartsCharts = dynamic(() => import("./RechartsWrapper"), {
  ssr: false,
  loading: () => (
    ...
    <div className="space-y-6">
          ...
          <div className="flex-1">
            ...
            <div className="bg-white rounded-lg p-3 mt-2">
              ...
              <ul className="text-sm text-gray-600 ml-4 list-disc space-y-1">
                <li>
                  {t.locale === "en"
                    ? "Can perform addition, subtraction, multiplication, division on encrypted data"
                    : "可以在加密数据上直接进行加、减、乘、除等运算"}
                </li>
                <li>
                  {t.locale === "en"
                    ? "Calculate total salary, average salary, employee count and other statistics"
                    : "计算总薪资、平均薪资、员工数量等统计信息"}
                </li>
                <li>
                  {t.locale === "en"
                    ? "No need to decrypt any individual employee salary data"
                    : "无需解密任何单个员工的薪资数据"}
                </li>
                <li>
                  {t.locale === "en"
                    ? "Ensure complete privacy protection while supporting data analysis"
                    : "确保完全的隐私保护，同时支持数据分析"}
                </li>
              </ul>
            </div>
      ...
      <div className="bg-gradient-to-r from-orange-500 to-red-600 rounded-xl shadow-lg p-6 text-white">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h3 className="text-xl font-bold mb-1">
              {t.locale === "en" ? "Fully Homomorphic Encryption Statistics" : "全同态加密统计"}
            </h3>
            <p className="text-orange-100">
              {t.locale === "en"
                ? "All calculations are performed on encrypted data without decrypting raw salary data"
        ...
        <div className="bg-white/20 rounded-lg p-4 mt-4">
          <p className="text-sm">
            ...
            {t.locale === "en"
              ? "Smart contracts can calculate total salary, average salary, employee count and other statistics, but cannot see any individual employee salary data. This ensures complete privacy protection!"
              : "智能合约可以计算总薪资、平均薪资、员工数等统计信息，但无法看到任何单个员工的薪资数据。这确保了完全的隐私保护！"}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">
          {t.locale === "en" ? "Select Department for Statistics" : "选择部门进行统计"}
        </h3>
        <div className="flex gap-4 items-end">
          <div className="flex-1">
            <label className="block text-sm font-medium text-gray-700 mb-2">{t.statistics.selectDepartment}</label>
            <select
        ...
        <div className="bg-orange-50 border-2 border-orange-300 rounded-lg p-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <p className="font-semibold text-orange-900 text-lg">
                {t.locale === "en"
                  ? "Calculating statistics (without decrypting raw data)"
                  : "正在计算统计（不解密原始数据）"}
              </p>
              <p className="text-sm text-orange-700 mt-1">
                {t.locale === "en"
                  ? "Steps: 🔐 Encrypted Data → ➕ Encrypted Addition → 📊 Encrypted Statistics → ✅ Decrypt Statistics Result"
          ...
          <div className="bg-white rounded-lg p-4 mt-4">
            ...
            <ul className="text-sm text-gray-600 mt-2 space-y-1 ml-4">
              <li>
                1. {t.locale === "en" ? "Read all employees' encrypted salary data" : "读取所有员工的加密薪资数据"}
              </li>
              <li>
                2.{" "}
                {t.locale === "en"
                  ? "Perform encrypted data addition without decryption (homomorphic addition)"
                  : "在不解密的情况下进行加密数据相加（同态加法）"}
              </li>
              <li>
                3. {t.locale === "en" ? "Calculate average (encrypted data division)" : "计算平均值（加密数据除法）"}
              </li>
              <li>4. {t.locale === "en" ? "Count employees" : "统计员工数量"}</li>
              <li>
                5.{" "}
                {t.locale === "en"
                  ? "Decrypt statistics result (only decrypt statistics value, not raw data)"
                  : "解密统计结果（只解密统计值，不解密原始数据）"}
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 mb-1">
                  {t.locale === "en" ? "Total Salary (Encrypted Calculation)" : "总薪资（加密计算）"}
                </p>
                <p className="text-3xl font-bold text-gray-900">${stats.totalSalary}</p>
                <p className="text-xs text-gray-500 mt-1">
                  🔐 {t.locale === "en" ? "No raw data decryption" : "不解密原始数据"}
                </p>
              </div>
              <div className="text-4xl">💰</div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-green-500">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 mb-1">
                  {t.locale === "en" ? "Average Salary (Encrypted Calculation)" : "平均薪资（加密计算）"}
                </p>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-purple-500">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  👥 {t.locale === "en" ? "Department Employees" : "部门员工"}
                </p>
              </div>
              <div className="text-4xl">👥</div>
            </div>
            ...
            onClick={async () => {
              const budgetInput = (
                document.querySelector(
                  `input[placeholder*="${t.locale === "en" ? "e.g." : "例如"}"]`,
                ) as HTMLInputElement
              )?.value;
                ...
                notification.success(
                  <div className="space-y-2">
                    <div className="font-bold">
                      ✅ {t.locale === "en" ? "Budget Compliance Check Complete" : "预算合规检查完成"}
                    </div>
                    <div className="text-sm">
                      <div className="mb-2">
                        {t.locale === "en"
                          ? "This function will compare the department's total salary with the budget without decrypting any salary data."
                          : "此功能将在不解密任何薪资数据的情况下，比较部门总薪资与预算。"}
                      </div>
                      <div className="text-xs text-gray-400">
                        {t.locale === "en"
                          ? "This feature will implement full smart contract calls in future versions."
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        <h4 className="font-semibold text-blue-900 mb-3">
          🔐 {t.locale === "en" ? "Fully Homomorphic Encryption Statistics Principle" : "全同态加密统计原理"}
        </h4>
        <div className="space-y-3 text-sm text-blue-800">
          <div className="bg-white rounded-lg p-3">
            <p className="font-semibold mb-1">
              {t.locale === "en" ? "Traditional Method (Requires Decryption):" : "传统方式（需要解密）："}
            </p>
            <p className="text-gray-700">
              {t.locale === "en"
                ? "Decrypt All Salaries → Calculate Statistics → Display Results"
                : "解密所有薪资 → 计算统计 → 显示结果"}
              <br />
          ...
          <div className="bg-white rounded-lg p-3">
            <p className="font-semibold mb-1">
              {t.locale === "en" ? "FHE Method (No Raw Data Decryption):" : "FHE 方式（不解密原始数据）："}
            </p>
            <p className="text-gray-700">
              {t.locale === "en"
                ? "Encrypted Data Direct Calculation → Decrypt Statistics Result"
                : "加密数据直接计算 → 解密统计结果"}
              <br />
          ...
          <div className="mt-4 p-3 bg-blue-100 rounded-lg">
            ...
            <ul className="list-disc list-inside mt-1 space-y-1 text-blue-800">
              <li>
                {t.locale === "en"
                  ? "Smart contracts cannot see any individual employee salary"
                  : "智能合约无法看到任何单个员工的薪资"}
              </li>
              <li>
                {t.locale === "en"
                  ? "Statistical calculations are performed on encrypted data"
                  : "统计计算在加密数据上进行"}
              </li>
              <li>
                {t.locale === "en"
                  ? "Only statistics results are decrypted, raw data remains encrypted"
                  : "只有统计结果被解密，原始数据保持加密"}
              </li>
              <li>
                {t.locale === "en" ? "Fully compliant with data protection regulations" : "完全符合数据保护法规要求"}
              </li>
            </ul>
          </div>
        </div>
