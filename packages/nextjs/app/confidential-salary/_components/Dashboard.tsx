    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600/90 to-indigo-700/90 backdrop-blur-md rounded-xl shadow-lg p-8 text-white border border-white/30">
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            ...
            <p className="text-blue-50 text-lg mb-4">{t.dashboard.description}</p>
          </div>
          {onStartGuide && (
            <button
        ...
        <div className="bg-white/20 rounded-lg p-4 mb-4">
          <p className="text-sm text-white">
            <strong className="text-blue-50">💡 {t.locale === "en" ? "Tip:" : "使用提示："}</strong>{" "}
            {t.dashboard?.tip ||
              (t.locale === "en"
                ? "Access all functional modules through the top navigation bar. Each module has detailed function descriptions and operation guides"
                : "通过顶部导航栏可以访问所有功能模块。每个模块都有详细的功能说明和操作指引")}
          </p>
        </div>
        <div className="mt-4 flex items-center gap-2 text-sm flex-wrap">
          <span className="bg-white/30 px-3 py-1 rounded-full text-white font-medium">
            ...
            {t.dashboard?.features?.encryption ||
              (t.locale === "en" ? "Fully Homomorphic Encryption Protection" : "全同态加密保护")}
          </span>
          <span className="bg-white/30 px-3 py-1 rounded-full text-white font-medium">
            👥{" "}
            {t.dashboard?.features?.permissions ||
              (t.locale === "en" ? "Role-Based Permission Management" : "基于角色的权限管理")}
          </span>
          <span className="bg-white/30 px-3 py-1 rounded-full text-white font-medium">
            📊{" "}
            {t.dashboard?.features?.statistics ||
              (t.locale === "en" ? "Encrypted Statistical Analysis" : "加密统计分析")}
          </span>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex-1">
            ...
            <p className="text-yellow-100 text-sm mb-2">
              {t.locale === "en"
                ? "Generate complete demo data with one click, experience all features immediately, no manual creation needed"
      ...
      <div className="bg-blue-500/20 backdrop-blur-md border border-blue-400/50 rounded-xl p-6">
        ...
        <p className="text-sm text-gray-200 mb-4">
          💡 <strong>{t.locale === "en" ? "Tip:" : "提示："}</strong>{" "}
          {t.locale === "en"
            ? "It is recommended to use 'Generate Demo Data' for quick experience first. If you need to create manually, follow these steps:"
            : '建议先使用"一键生成演示数据"快速体验，如需手动创建可按以下步骤：'}
        </p>
          ...
          <div className="flex items-start bg-white/5 rounded-lg p-3">
            ...
            <div>
              ...
              <p className="text-gray-300 text-xs">
                {t.locale === "en" ? "Department Management → Create Department" : "部门管理 → 创建部门"}
              </p>
            </div>
          </div>
          <div className="flex items-start bg-white/5 rounded-lg p-3">
            ...
            <div>
              ...
              <p className="text-gray-300 text-xs">
                {t.locale === "en" ? "Employee Management → Add Employee" : "员工管理 → 添加员工"}
              </p>
            </div>
          </div>
          <div className="flex items-start bg-white/5 rounded-lg p-3">
            ...
            <div>
              ...
              <p className="text-gray-300 text-xs">
                {t.locale === "en" ? "Salary Management → Submit Salary" : "薪资管理 → 提交薪资"}
              </p>
            </div>
          </div>
          <div className="flex items-start bg-white/5 rounded-lg p-3">
            ...
            <div>
              ...
              <p className="text-gray-300 text-xs">
                {t.locale === "en" ? "Statistical Analysis → View Results" : "统计分析 → 查看结果"}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white/10 backdrop-blur-md rounded-lg shadow-md p-6 border border-white/20">
        ...
        <p className="text-sm text-gray-200 mb-4">
          {t.locale === "en"
            ? "Click the buttons below to quickly jump to corresponding functional modules"
            : "点击下方按钮快速跳转到对应功能模块"}
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            ...
            <div className="text-sm text-gray-300">
              {t.locale === "en" ? "Add new department and set budget" : "添加新部门并设置预算"}
            </div>
          </button>
          ...
          >
            ...
            <div className="text-sm text-gray-300">
              {t.locale === "en" ? "Register new employee and assign role" : "注册新员工并分配角色"}
            </div>
          </button>
          ...
          >
            ...
            <div className="text-sm text-gray-300">
              {t.locale === "en" ? "Submit salary using FHE encryption" : "使用FHE加密提交薪资"}
            </div>
          </button>
          ...
          >
            ...
            <div className="text-sm text-gray-300">
              {t.locale === "en" ? "Encrypted data statistical analysis" : "加密数据统计分析"}
            </div>
          </button>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-br from-indigo-500/30 to-purple-500/30 backdrop-blur-md rounded-xl shadow-lg p-6 border border-white/20">
        <h3 className="text-xl font-bold text-white mb-4">
          ✨ {t.locale === "en" ? "Core Feature Highlights" : "核心功能亮点"}
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20 hover:bg-white/15 transition-all">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h4 className="font-semibold text-white">
                {t.locale === "en" ? "Fully Homomorphic Encryption" : "全同态加密"}
              </h4>
            </div>
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20 hover:bg-white/15 transition-all">
            <div className="flex items-center gap-3 mb-2">
              <span className="text-2xl">👥</span>
              <h4 className="font-semibold text-white">
                {t.locale === "en" ? "Role Permission Management" : "角色权限管理"}
              </h4>
            </div>
            <p className="text-sm text-gray-200">
              {t.locale === "en"
                ? "Smart contract-based RBAC system, ensuring data security and access control"
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20 hover:bg-white/15 transition-all">
            <div className="flex items-center gap-3 mb-2">
              <span className="text-2xl">🏢</span>
              <h4 className="font-semibold text-white">
                {t.locale === "en" ? "Enterprise Application" : "企业级应用"}
              </h4>
            </div>
            <p className="text-sm text-gray-200">
              {t.locale === "en"
