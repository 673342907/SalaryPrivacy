  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">{t.locale === "en" ? "Current User Role" : "当前用户角色"}</h3>
        <div className="flex items-center gap-4">
          <div className="text-3xl">👤</div>
          <div className="flex-1">
            <p className="text-sm text-gray-600 mb-1">{t.locale === "en" ? "Wallet Address" : "钱包地址"}</p>
            <p className="font-mono text-sm text-gray-900">
              {address ? `${address.slice(0, 10)}...${address.slice(-8)}` : (t.locale === "en" ? "Not connected" : "未连接")}
            </p>
          </div>
          <div>
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">{t.locale === "en" ? "Role Permission Comparison" : "角色权限对比"}</h3>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              <tr>
                ...
                {roles.map((role) => (
                  <td key={role.id} className="px-6 py-4 whitespace-nowrap text-center">
                    {role.permissions.some(p => t.locale === "en" ? p.includes("Create and manage departments") : p.includes("创建和管理部门")) ? (
              ...
              <tr>
                ...
                {roles.map((role) => (
                  <td key={role.id} className="px-6 py-4 whitespace-nowrap text-center">
                    {role.permissions.some(p => t.locale === "en" ? p.includes("Add and manage employees") : p.includes("添加和管理员工")) ? (
                      <span className="text-green-600 text-xl">✅</span>
                    ) : (
                      <span className="text-gray-400 text-xl">❌</span>
                    )}
              ...
              <tr>
                ...
                {roles.map((role) => (
                  <td key={role.id} className="px-6 py-4 whitespace-nowrap text-center">
                    {role.permissions.some(p => t.locale === "en" ? p.includes("statistics") || p.includes("Statistics") : p.includes("统计")) ? (
                      <span className="text-green-600 text-xl">✅</span>
                    ) : (
                      <span className="text-gray-400 text-xl">❌</span>
                    )}
              ...
              <tr>
                ...
                {roles.map((role) => (
                  <td key={role.id} className="px-6 py-4 whitespace-nowrap text-center">
                    {role.permissions.some(p => t.locale === "en" ? p.includes("salary") || p.includes("Salary") : p.includes("薪资")) ? (
                      <span className="text-green-600 text-xl">✅</span>
                    ) : (
                      <span className="text-gray-400 text-xl">❌</span>
                    )}
      ...
      <div className="bg-red-50 border border-red-200 rounded-lg p-6">
        <h4 className="font-semibold text-red-900 mb-3">🔐 {t.locale === "en" ? "Permission System Description" : "权限系统说明"}</h4>
        <div className="space-y-3 text-sm text-red-800">
          <div>
            <p className="font-semibold mb-1">{t.locale === "en" ? "Smart Contract-Based Permission Control:" : "基于智能合约的权限控制："}</p>
            <ul className="list-disc list-inside space-y-1 text-red-700">
              <li>{t.locale === "en" ? "All permission checks are executed on-chain, tamper-proof" : "所有权限检查都在链上执行，不可篡改"}</li>
              <li>{t.locale === "en" ? "Only Admin can assign roles" : "只有 Admin 可以分配角色"}</li>
              <li>{t.locale === "en" ? "Role assignments are recorded on blockchain, auditable" : "角色分配记录在区块链上，可审计"}</li>
              <li>{t.locale === "en" ? "Permission checks are automatically executed on each operation" : "权限检查在每次操作时自动执行"}</li>
            </ul>
          </div>
          <div className="mt-4 p-3 bg-white rounded-lg">
            <p className="font-semibold text-red-900 mb-1">{t.locale === "en" ? "Permission Verification Flow:" : "权限验证流程："}</p>
            <p className="text-red-700">
              {t.locale === "en" ? "User Operation → Smart Contract Checks Role → Verify Permission → Execute or Reject" : "用户操作 → 智能合约检查角色 → 验证权限 → 执行操作或拒绝"}
            </p>
          </div>
        </div>
      </div>
