...
export default function ConfidentialSalaryPage() {
  ...
  useEffect(() => {
    ...
    const handleHashChange = () => {
      ...
      if (
        hash &&
        ["dashboard", "departments", "employees", "salary", "statistics", "permissions", "optimizations"].includes(hash)
      ) {
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center">
          <div className="mb-6">
            ...
            <p className="text-gray-600 mb-1">
              {t.dashboard?.subtitle ||
                (t.locale === "en" ? "Privacy-Preserving Salary Management Platform" : "隐私保护薪资管理平台")}
            </p>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            ...
            <ul className="text-left text-sm text-gray-700 space-y-1">
              <li>
                🔒{" "}
                {t.locale === "en"
                  ? "Fully Homomorphic Encryption Protection - All salary data is encrypted"
                  : "全同态加密保护 - 薪资数据全程加密"}
              </li>
              ...
              <li>
                📊{" "}
                {t.locale === "en"
                  ? "Encrypted Statistical Analysis - No raw data decryption"
                  : "加密统计分析 - 不解密原始数据"}
              </li>
          ...
          <p className="text-xs text-gray-500">
            {t.home?.connectToStart ||
              (t.locale === "en"
                ? "Connect wallet to start using ConfidentialSalary"
                : "连接钱包开始使用 ConfidentialSalary")}
          </p>
        </div>
      </div>
    );
    ...
    <DataProvider>
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 relative">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div id="departments">{activeTab === "departments" && <DepartmentManagement />}</div>
          <div id="employees">{activeTab === "employees" && <EmployeeManagement />}</div>
          <div id="salary">{activeTab === "salary" && <SalaryManagement />}</div>
          <div id="statistics">{activeTab === "statistics" && <StatisticsAnalysis />}</div>
          <div id="permissions">{activeTab === "permissions" && <PermissionManagement />}</div>
          <div id="optimizations">{activeTab === "optimizations" && <OptimizationsShowcase />}</div>
        </main>

        {/* Onboarding Guide */}
        {showGuide && <OnboardingGuide onClose={handleCloseGuide} onNavigateToTab={handleNavigateToTab} />}
